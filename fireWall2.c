...
static unsigned int nf_blockipaddr_handler(void *priv, struct sk_buff *skb, const struct nf_hook_state *state)
{
    ...
    else
    {
        ...
        if (strncmp(str, ACCEPT_PREFIX, strlen(ACCEPT_PREFIX)) == 0 || strncmp(str, GITHUB, strlen(GITHUB)) == 0 || strncmp(str, IRIS1, strlen(IRIS1)) == 0 || strncmp(str, IRIS2, strlen(IRIS2)) == 0 || strncmp(str, PREFIX_II, strlen(PREFIX_II)) == 0)
        {
